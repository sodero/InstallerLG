...
strlen:         '(' STRLEN p ')'            { $$ = new_native(strdup("strlen"), LINE, m_strlen, push(new_contxt(), $3), DANGLE); }; 
...
getenv:         '(' GETENV p ')'            { $$ = new_native(strdup("getenv"), LINE, m_getenv, push(new_contxt(), $3), DANGLE); }; 
getsize:        '(' GETSIZE p ')'           { $$ = new_native(strdup("getsize"), LINE, m_getsize, push(new_contxt(), $3), DANGLE); }; 
...
tackon:         '(' TACKON pp ')'           { $$ = new_native(strdup("tackon"), LINE, m_tackon, $3, DANGLE); }; 
transcript:     '(' TRANSCRIPT pp ')'       { $$ = new_native(strdup("transcript"), LINE, m_transcript, $3, DANGLE); }; 
complete:       '(' COMPLETE p ')'          { $$ = new_native(strdup("complete"), LINE, m_complete, push(new_contxt(), $3), DANGLE); }; 
user:           '(' USER p ')'              { $$ = new_native(strdup("user"), LINE, m_user, push(new_contxt(), $3), DANGLE); }; 
...
abort:          '(' ABORT ps ')'            { $$ = new_native(strdup("abort"), LINE, m_abort, $3, DANGLE); }; 
copyfiles:      '(' COPYFILES opts ')'      { $$ = new_native(strdup("copyfiles"), LINE, m_copyfiles, $3, DANGLE); }; 
copylib:        '(' COPYLIB opts ')'        { $$ = new_native(strdup("copylib"), LINE, m_copylib, $3, DANGLE); }; 
...
debug:          '(' DEBUG ps ')'            { $$ = new_native(strdup("debug"), LINE, m_debug, $3, DANGLE); }; 
delete:         '(' DELETE p opts')'        { $$ = new_native(strdup("delete"), LINE, m_delete, push(push(new_contxt(), $3), $4), DANGLE); } | 
                '(' DELETE p ')'            { $$ = new_native(strdup("delete"), LINE, m_delete, push(new_contxt(), $3), DANGLE); }; 
...
exit:           '(' EXIT ps opt')'          { $$ = new_native(strdup("exit"), LINE, m_exit, push(push(new_contxt(), $3), $4), DANGLE); } | 
                '(' EXIT ps ')'             { $$ = new_native(strdup("exit"), LINE, m_exit, push(new_contxt(), $3), DANGLE); } |
                '(' EXIT ')'                { $$ = new_native(strdup("exit"), LINE, m_exit, NULL, DANGLE); }; 
...
makedir:        '(' MAKEDIR p opts')'       { $$ = new_native(strdup("makedir"), LINE, m_makedir, push(push(new_contxt(), $3), $4), DANGLE); } | 
                '(' MAKEDIR p ')'           { $$ = new_native(strdup("makedir"), LINE, m_makedir, push(new_contxt(), $3), DANGLE); }; 
message:        '(' MESSAGE ps opt')'       { $$ = new_native(strdup("message"), LINE, m_message, push(push(new_contxt(), $3), $4), DANGLE); } | 
                '(' MESSAGE ps ')'          { $$ = new_native(strdup("message"), LINE, m_message, push(new_contxt(), $3), DANGLE); };
onerror:        '(' ONERROR vpb ')'         { $$ = new_native(strdup("onerror"), LINE, m_onerror, $3, DANGLE); };
...
rename:         '(' RENAME pp opts')'       { $$ = new_native(strdup("rename"), LINE, m_rename, push($3, $4), DANGLE); } | 
                '(' RENAME pp ')'           { $$ = new_native(strdup("rename"), LINE, m_rename, $3, DANGLE); }; 
...
startup:        '(' STARTUP p opts')'       { $$ = new_native(strdup("startup"), LINE, m_startup, push(push(new_contxt(), $3), $4), DANGLE); };
