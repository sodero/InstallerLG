...
foreach:        '(' FOREACH pp vpb ')'          { $$ = new_native(strdup("foreach"), LINE, m_foreach, push($3, $4), NUMBER); };
makeassign:     '(' MAKEASSIGN pp opt')'        { $$ = new_native(strdup("makeassign"), LINE, m_makeassign, push($3, $4), NUMBER); } | 
                '(' MAKEASSIGN pp ')'           { $$ = new_native(strdup("makeassign"), LINE, m_makeassign, $3, NUMBER); } | 
                '(' MAKEASSIGN p opt ')'        { $$ = new_native(strdup("makeassign"), LINE, m_makeassign, push(push(new_contxt(), $3), $4), NUMBER); } | 
